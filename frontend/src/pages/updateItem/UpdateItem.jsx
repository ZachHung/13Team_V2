...
function UpdateItem() {
    ...
    return (
        <div className="container mt-4 mb-4">
            ...
            {item.map((item) => (
                <form className="mt-4" method="POST" action={URL + "admin/products/update/" + item._id}>
                    <a href={`/admin/products/updateDetail/${item._id}`} className="btn btn-primary bnt-more">Sửa chi tiết</a>
                    <br></br>
                    <div className="mb-4">
                        <label htmlFor="name" className="form-label lable_level_1">Tên sản phẩm</label>
                        <input type="text" className="form-control my-input-tag" defaultValue={item.name} id="name" name="name" />

                    </div>

                    <div className="mb-4">
                        <label htmlFor="type" className="form-label lable_level_1">Loại sản phẩm</label>
                        <input type="text" className="form-control my-input-tag" defaultValue={item.type} id="type" name="type" />
                    </div>

                    <div className="mb-4">

                        <label className="form-label lable_level_1 mb-0">Hình ảnh sản phẩm <a id='add-image' onClick={AddImageShow} className="btn btn-primary addbnt">+</a></label>
                    ...
                    <div className="mb-4">
                        <label htmlFor="description" className="form-label lable_level_1">Mô tả sản phẩm</label>
                        <textarea className="form-control my-area-tag" id="description" name="description" defaultValue={item.description} />
                    </div>

                    <label className="lable_level_1">Thương hiệu</label>
                    <div class="lavel_2">
                        <div className="mb-4">
                            <label htmlFor="name" className="form-label lable_level_2">Tên thương hiệu</label>
                            <input type="text" className="form-control my-input-tag" defaultValue={item.brand.name} id="brand.name" name="brand.name" />
                        </div>

                        <div className="mb-4">
                            <label htmlFor="brandImage" className="form-label lable_level_2">Ảnh thương hiệu</label>
                    ...
                    <div className="mb-4">
                        <h1 className="form-label lable_level_1">Thông tin về công nghệ</h1>
                        {item.techInfo?.map((techInfos, index_1) => (
                            <div className='lavel_2'>
                                <lable className="form-label lable_level_2">{techInfos.infoType}</lable>
                                {techInfos.infoDetail?.map((infoDetails, index_2) => (
                                    <div className='mb-4'>
                                        <lable className="form-label lable_level_3">{infoDetails.infoName}</lable>
