...
function DetailPurchaseAdmin() {
  ...
  return (
    <div className='d-flex flex-column marginTop'>
      <Link className='btn btnback' to='/admin/orders/'>
        &#8592; Quay lại
      </Link>
      ...
      {purchaseDetail?.map((purchase) => (
        <div
          className='purCart text-white'
          style={{ border: "2px solid #0c517d" }}
        >
          <div className='p-3'>
            ...
            <div className='purchaseInfo d-flex bg-white text-dark p-3 justify-content-between'>
              <div>
                ...
                <p className='px-2'>
                  <b>Địa chỉ: </b>
                  {purchase.userID.address.addressdetail === ""
                    ? ""
                    : `${purchase.userID.address.addressdetail},`}{" "}
                  {purchase.userID.address.ward === ""
                    ? ""
                    : `${purchase.userID.address.ward},`}{" "}
                  {purchase.userID.address.district === ""
                    ? ""
                    : `${purchase.userID.address.district},`}{" "}
                  {purchase.userID.address.province === ""
                    ? ""
                    : `${purchase.userID.address.province}`}
                </p>
              </div>
              ...
              <table className='table tableDetail'>
                <thead>
                  <tr className='main-heading h4'>
                    ...
                    <th style={{ width: "auto", height: "auto" }}>
                      Thành tiền
                    </th>
                  </tr>
                </thead>
                    ...
                    purchaseDe.list?.map((listOpts) => (
                      <tr className='cake-top' key={listOpts._id}>
                        ...
                        <td className='cake-text align-middle'>
                          <a
                            href={`/admin/products/detail/${0}`}
                            className='align-middle h5'
                          >
                            {listOpts.optionID.item.name}
                          </a>
                        </td>

                        <td className='align-middle h5'>{listOpts.color}</td>
                        <td className='align-middle h5'>{listOpts.quantity}</td>
